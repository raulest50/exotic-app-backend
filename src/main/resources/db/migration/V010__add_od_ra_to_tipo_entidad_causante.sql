-- Migración: Agregar OD_RA (Dispensación de Reposición por Avería) a tipo_entidad_causante
-- Fecha: 2026-03-05
-- Descripción: Actualiza el constraint check de transaccion_almacen para permitir
--              el nuevo valor OD_RA (ordinal 8) en tipo_entidad_causante.
--
...
--              7 = RAA   (Reporte de Avería de Almacén)
--              8 = OD_RA (Dispensación de Reposición por Avería) -- NUEVO

-- Eliminar el constraint existente
ALTER TABLE transaccion_almacen
DROP CONSTRAINT IF EXISTS transaccion_almacen_tipo_entidad_causante_check;

-- Crear el nuevo constraint que permite valores de 0 a 8
ALTER TABLE transaccion_almacen
ADD CONSTRAINT transaccion_almacen_tipo_entidad_causante_check
CHECK (tipo_entidad_causante >= 0 AND tipo_entidad_causante <= 8);
